...
messageRoutes.route('/messages/getHistory').post(async (req, res) => {
    let arr = [con.escape(req.body.user1), con.escape(req.body.user2)]
    arr.sort()
...
messageRoutes.route('/messages/sendMessage').post(async (req, res) => {
    let arr = [con.escape(req.body.fromUser), con.escape(req.body.toUser)]
    arr.sort()
    ...
    sql = `INSERT INTO Messages VALUES('${con.escape(req.body.fromUser)}', '${con.escape(req.body.toUser)}', '${con.escape(req.body.message)}', ${result[0].ConversationID}, NOW()) `
    result = await con.awaitQuery(sql)
...
messageRoutes.route('/messages/deleteConvo').post(async (req, res) => {
    ...
    let arr = [con.escape(req.body.currentUser), con.escape(req.body.deletedUser)]
    arr.sort()
    ...
    sql = `INSERT INTO DeleteConversations VALUES('${con.escape(req.body.currentUser)}', ${result[0].ConversationID})`
    result = await con.awaitQuery(sql)
    res.json(result)
...
messageRoutes.route('/messages/getConversations').post(async (req, res) => {
    ...
    let sql = `SELECT fromUser, toUser, message, timeStamp FROM Messages WHERE conversationID IN (SELECT ConversationID FROM Conversations WHERE (user1 = '${con.escape(req.body.user)}' OR user2 = '${con.escape(req.body.user)}')) AND (conversationID, timestamp) IN (SELECT conversationID,
        MAX(timeStamp) FROM Messages GROUP BY(conversationID)) AND conversationID NOT IN (SELECT conversationID FROM DeletedConversations WHERE username = '${con.escape(req.body.user)}') AND (fromUser, toUser) NOT IN (SELECT userBlocking, userBlocked FROM Block) AND (fromUser, toUser) NOT IN (SELECT userBlocked, userBlocking FROM Block) ORDER BY timeStamp DESC;`
